...
registerUIHooks(
    ...
    (success) => { // debrief
        ...
        let margin = Math.max(0, 100 - (currentAct.maxStress / 250) * 100);
...
function init() {
    ...
    canvasMain.addEventListener('mouseup', () => { mouse.down = false; mouse.right = false; });
...
function resize() {
    ...
    setDimensions(w, h);
    if (glCanvasMain) { glCanvasMain.width = w; glCanvasMain.height = h; }
}
